...
type Client interface {
	...
	LocateResource2(ResourceName, string, uint32) (string, error)
}
...
// LocateResource2 returns address of the server for the specified resource.
func (c *client) LocateResource2(resource ResourceName, mip string, mport uint32) (string, error) {
	conn, _, err := zk.Connect(c.zks, c.sessionTimeout)
	if err != nil {
		return "", fmt.Errorf("error connecting to ZooKeeper at %v: %s", c.zks, err)
	}
